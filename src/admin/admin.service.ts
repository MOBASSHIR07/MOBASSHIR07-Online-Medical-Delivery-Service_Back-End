...
async updateAdminByEmail(email: string, adminDTO: AdminDTO): Promise<any> {
  // First, you might want to find the admin by email
  ...
  // Now update the found admin with the data from adminDTO
  // You can use something like Object.assign or spread operator to merge the properties
  const updatedAdmin = { ...admin, ...adminDTO };
...
async getimagebyadminid(id): Promise<string> {
  const mydata: AdminDTO = await this.adminRepo.findOneBy({userId: id});
  console.log(mydata);
  return mydata.profilePicture;
}
...
async getimagebyemail(email: string): Promise<string> {
  ...
  const adminData: AdminEntity = await this.adminRepo.findOne({ where: { email } });

  if (!adminData) {
      throw new NotFoundException('Admin not found'); // Handle case where admin with the provided email is not found
  }

  // Now that you have the userId, fetch the profilePicture
  const profilePicture = adminData.profilePicture;
