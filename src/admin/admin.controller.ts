...
Param, UseGuards, Delete, Res,   ParseIntPipe, HttpException, HttpStatus,NotFoundException,
...
async updateAdminByEmail(@Param('email') email: string, @Body() adminDTO: AdminDTO): Promise<any> {
  return this.adminService.updateAdminByEmail(email, adminDTO);
}
...
@Get('getimagebyadminid/:id')
async getimagebyid(@Param('id', ParseIntPipe) adminId: number, @Res() res) {
...
@Get('getusers/:email')
    ...
    // @Get('getimagebyadminemail/:email')
    // @UseInterceptors(FileInterceptor('myfile'))
    // async getimagebyemail(@Param('email') email: string): Promise<string> {
    //     try {
    //         return this.adminService.getimagebyemail(email);
    //     } catch (error) {
    //         throw new NotFoundException('File not found'); // Adjust error handling as needed
    //     }
    // }
    @Get('getimagebyadminemail/:email')
    async getimagebyemail(@Param('email') email: string, @Res() res: Response): Promise<void> {
        try {
            const filename = await this.adminService.getimagebyemail(email);
            res.sendFile(filename, { root: './upload' }); // Assuming the file is stored in the ./upload directory
        } catch (error) {
            throw new NotFoundException('File not found'); // Adjust error handling as needed
        }
    }
